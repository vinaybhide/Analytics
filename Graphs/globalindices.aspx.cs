...
namespace Analytics
{
    public partial class globalindices : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Session["EmailId"] != null)
            {
                ...
                if (panelWidth.Value != "" && panelHeight.Value != "")
                {
                    ...
                    chartdailyIndices.Width = int.Parse(panelWidth.Value);
                    chartdailyIndices.Height = int.Parse(panelHeight.Value);
                }
            }
            else
            {
        ...
        {
            //checkboxlistLines.Visible = false;
            //return;
            checkboxlistLines.Visible = true;
            ListItem li = new ListItem("BSE SENSEX", "^BSESN");
            li.Selected = true;
            checkboxlistLines.Items.Add(li);
                ...
                foreach (ListItem item in checkboxlistLines.Items)
                {
                    if (item.Selected == true)
                    {
                        ...
                        if (scriptData != null)
                        {
                            ...
                            chartdailyIndices.Series[item.Value].PostBackValue = item.Text + ",#VALX,#VALY1,#VALY2,#VALY3,#VALY4,#:VALY5";

                            chartdailyIndices.Series[item.Value].CustomProperties = "PriceDownColor=Blue, ShowOpenClose=Both, PriceUpColor=Red, OpenCloseStyle=Traingle";
